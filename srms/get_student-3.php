...
if (!empty($_POST["classid"])) {
    ...
    if (!is_numeric($cid)) {
        ...
        $stmt = $dbh->prepare("SELECT StudentName,StudentId FROM tblstudents3 WHERE ClassId= :id ORDER BY StudentName");
...
if (!empty($_POST["classid1"])) {
    ...
    if (!is_numeric($cid1)) {
        ...
        $status = 0;
        $stmt = $dbh->prepare("SELECT tblsubjects3.SubjectName, tblsubjects3.id FROM tblsubjectcombination3 JOIN tblsubjects3 ON tblsubjects3.id=tblsubjectcombination3.SubjectId WHERE tblsubjectcombination3.ClassId=:cid AND tblsubjectcombination3.status!=:stts ORDER BY tblsubjects3.SubjectName");
...
if (!empty($_POST["studclass"])) {
    ...
    $id = $dta[0];
    $id1 = $dta[1];
    $query = $dbh->prepare("SELECT StudentId,ClassId FROM tblresult3 WHERE StudentId=:id1 AND ClassId=:id ");
