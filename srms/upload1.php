        ...
        try {
            ...
            if ($resultTable) {
                ...
                foreach ($worksheet->getRowIterator() as $row) {
                    ...
                    $sqlCheckStudent = "SELECT StudentId FROM tblstudents WHERE RollId = :rollId";
                    ...
                    if (!$existingStudent) {
                        ...
                        $sqlInsertClass = "INSERT INTO tblclasses (ClassNameNumeric, ClassName)
                        ...
                        if ($existingStudent) {
                            ...
                            $sqlStudents = "UPDATE tblstudents SET StudentName = :studentName WHERE StudentId = :studentId";
                            ...
                            // Insert new student
                            $sqlStudents = "INSERT INTO tblstudents (StudentName, RollId, ClassId, Status )
                        ...
                        for ($i = 4; $i < count($rowData); $i += 2) {
                            $subjectName = $rowData[$i];
                            $marks = $rowData[$i + 1];

                            // Check if the subject already exists in tblsubject
                            if (!isset($subjectIdMap[$subjectName])) {
                                $sqlCheckSubject = "SELECT SubjectId FROM tblsubject WHERE SubjectName = :subjectName";
                                $queryCheckSubject = $dbh->prepare($sqlCheckSubject);
                                $queryCheckSubject->bindParam(':subjectName', $subjectName, PDO::PARAM_STR);
                                $queryCheckSubject->execute();
                                $existingSubject = $queryCheckSubject->fetch(PDO::FETCH_ASSOC);

                                if ($existingSubject) {
                                    // Subject already exists, retrieve its SubjectId
                                    $subjectIdMap[$subjectName] = $existingSubject['SubjectId'];
                                } else {
                                    // Subject does not exist, insert it
                                    $sqlInsertSubject = "INSERT INTO tblsubject (SubjectName) VALUES (:subjectName)";
                                    $queryInsertSubject = $dbh->prepare($sqlInsertSubject);
                                    $queryInsertSubject->bindParam(':subjectName', $subjectName, PDO::PARAM_STR);
                                    $queryInsertSubject->execute();

                                    // Get the SubjectId
                                    $subjectIdMap[$subjectName] = $dbh->lastInsertId();
                                }
                            }

                            // Insert or update marks in result table
                            $subjectId = $subjectIdMap[$subjectName];

                            $sqlCheckResult = "SELECT ResultId FROM $resultTable WHERE StudentId = :studentId AND SubjectId = :subjectId";
                            $queryCheckResult = $dbh->prepare($sqlCheckResult);
                            $queryCheckResult->bindParam(':studentId', $studentId, PDO::PARAM_INT);
                            $queryCheckResult->bindParam(':subjectId', $subjectId, PDO::PARAM_INT);
                            $queryCheckResult->execute();
                            $existingResult = $queryCheckResult->fetch(PDO::FETCH_ASSOC);

                            if ($existingResult) {
                                // Update existing result
                                $resultId = $existingResult['ResultId'];
                                $sqlUpdateResult = "UPDATE $resultTable SET Marks = :marks WHERE ResultId = :resultId";
                                $queryUpdateResult = $dbh->prepare($sqlUpdateResult);
                                $queryUpdateResult->bindParam(':marks', $marks, PDO::PARAM_INT);
                                $queryUpdateResult->bindParam(':resultId', $resultId, PDO::PARAM_INT);
                                $queryUpdateResult->execute();
                            } else {
                                // Insert new result
                                $sqlInsertResult = "INSERT INTO $resultTable (StudentId, SubjectId, Marks)
                                                    VALUES (:studentId, :subjectId, :marks)";
                                $queryInsertResult = $dbh->prepare($sqlInsertResult);
                                $queryInsertResult->bindParam(':studentId', $studentId, PDO::PARAM_INT);
                                $queryInsertResult->bindParam(':subjectId', $subjectId, PDO::PARAM_INT);
                                $queryInsertResult->bindParam(':marks', $marks, PDO::PARAM_INT);
                                $queryInsertResult->execute();
                            }
                        }
                    }
                }
